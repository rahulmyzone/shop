...
public class Product extends SalesManagerEntity<Long, Product> implements Auditable {
	...
	//@ManyToOne(targetEntity = PromotionalVoucher.class)
	//@JoinColumn(name = "PROMOTIONAL_VOUCHER_ID", nullable = true)
	//private PromotionalVoucher promotionalVoucher;
	@ManyToMany(cascade=CascadeType.REFRESH, mappedBy="products")
	...
	@Transient
	private Set<ProductImage> menuImages = new HashSet<ProductImage>();
	...
	public ProductImage getProductImage() {
		...
		if(this.getImages()!=null && this.getImages().size()>0) {
			for(ProductImage image : this.getImages()) {
				if(image != null && image.getImageType() != null && image.getImageType() == ImageType.PRODUCT){
					productImage = image;
				}
	...
	public void setMenuImages(Set<ProductImage> menuImages) {
