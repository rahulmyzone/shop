...
@Controller
public class ProductMenuImagesController {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(ProductMenuImagesController.class);
	...
	@RequestMapping(value="/admin/products/images/menu/list.html", method=RequestMethod.GET)
	public String displayProductImages(@RequestParam("id") long productId, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		return ControllerConstants.Tiles.Product.productMenuImages;
	...
	@PreAuthorize("hasRole('PRODUCTS')")
	@RequestMapping(value="/admin/products/images/menu/page.html", method=RequestMethod.POST, produces="application/json")
	public @ResponseBody String pageProductImages(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			Set<ProductImage> images = product.getMenuImages();
	...
	@PreAuthorize("hasRole('PRODUCTS')")
	@RequestMapping(value="/admin/products/images/menu/save.html", method=RequestMethod.POST)
	public String saveProductImages(@ModelAttribute(value="menuImages") @Valid final ProductImages productImages, final BindingResult bindingResult,final Model model, final HttpServletRequest request,Locale locale) throws Exception{
		...
		model.addAttribute("product",product);
		if(product==null) {
			FieldError error = new FieldError("menuImages","image",messages.getMessage("message.error", locale));
        ...
        if(CollectionUtils.isNotEmpty( productImages.getFile() )){
            ...
            for(final MultipartFile multipartFile:productImages.getFile()){
                if(!multipartFile.isEmpty()){
                	...
                	productImage.setImageType(ImageType.MENU);
	...
	@PreAuthorize("hasRole('PRODUCTS')")
	@RequestMapping(value="/admin/products/images/menu/remove.html", method=RequestMethod.POST, produces="application/json")
