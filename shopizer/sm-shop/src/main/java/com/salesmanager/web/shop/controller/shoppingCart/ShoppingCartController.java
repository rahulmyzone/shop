...
public class ShoppingCartController extends AbstractController {
	...
	public @ResponseBody String checkoutPage(final HttpServletRequest request, final HttpServletResponse response){
		AjaxResponse resp = new AjaxResponse();
		 ...
		 */
		if(customer == null){
			resp.addEntry("gotoCart","1");
			resp.addEntry("redirectUrl", "/shop/customer/customLogon.html");
			resp.setStatus(AjaxResponse.RESPONSE_STATUS_SUCCESS);
			super.setSessionAttribute("gotoCart", "1", request);
		}else{
			resp.addEntry("gotoCart","0");
			resp.addEntry("redirectUrl", "/shop/order/checkout.html");
			resp.setStatus(AjaxResponse.RESPONSE_STATUS_SUCCESS);
		}
	...
	/**
	 * Redirects user to appropriate page from shopping cart
	 * @param request
	 * @param response
	 * @return
	 */
	@RequestMapping(value={"/redirectCart.html"},  method = { RequestMethod.GET })
	public String redirectCheckout(final HttpServletRequest request, final HttpServletResponse response){
			String redirectUrl = null;
			Customer customer = super.getSessionAttribute(Constants.CUSTOMER, request);
			/**
			 * User not logged in
			 ...
			 */
			if(customer == null){
				redirectUrl = "redirect:/shop/customer/customLogon.html";
				super.setSessionAttribute("gotoCart", "1", request);
			}else{
				redirectUrl = "redirect:/shop/order/checkout.html";
			}
		return redirectUrl;
	}
