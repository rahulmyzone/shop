...
import com.salesmanager.core.business.payments.model.AtomPayment;
import com.salesmanager.core.business.payments.model.PaymentType;
import com.salesmanager.core.business.payments.model.PaypalPayment;
import com.salesmanager.core.business.payments.model.PaytmPayment;
...
import com.salesmanager.core.modules.integration.payment.impl.AtomCheckout;
...
import com.salesmanager.web.entity.order.Order;
...
public class ShoppingOrderPaymentController extends AbstractController {
	...
	public @ResponseBody String paymentAction(@Valid @ModelAttribute(value="order") ShopOrder order, @PathVariable String action, @PathVariable String paymentmethod, Device device, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
		...
		try {
			...
			order.getCustomer().getBilling().setZone("IND");
			//order.setcust
			...
			if(action.equals(INIT_ACTION)) {
				if(paymentmethod.contentEquals("PAYTM")){
					try{
						...
						paytmPayment.setCurrency(store.getCurrency());
						paytmPayment.setModuleName("paytm");
						paytmPayment.setPaymentType(PaymentType.PAYTM);
						paytmPayment.setTransactionType(TransactionType.CAPTURE);
						Customer customer = (Customer)request.getSession().getAttribute(Constants.CUSTOMER);
						BigDecimal total = orderTotalSummary.getTotal();
						Transaction transaction = paytmCheckout.initTransaction(store, customer, total, paytmPayment, config, integrationModule);
						if(transaction != null){
							...
							ajaxResponse.addDataEntry(transaction.getTransactionDetails());
				...
				}if(paymentmethod.equals("ATOM")){
					try{
						PaymentModule module = paymentService.getPaymentModule("atom");
						AtomCheckout atomCheckout = (AtomCheckout) module;
						AtomPayment atomPayment = new AtomPayment();
						atomPayment.setCurrency(store.getCurrency());
						Customer customer = (Customer)request.getSession().getAttribute(Constants.CUSTOMER);
						BigDecimal total = orderTotalSummary.getTotal();
						Transaction transaction = atomCheckout.initTransaction(store, customer, total, atomPayment, config, integrationModule);
						if(transaction != null){
							transactionService.create(transaction);
							super.setSessionAttribute(Constants.INIT_TRANSACTION_KEY, transaction, request);
							super.setSessionAttribute(Constants.ORDER, order, request);
