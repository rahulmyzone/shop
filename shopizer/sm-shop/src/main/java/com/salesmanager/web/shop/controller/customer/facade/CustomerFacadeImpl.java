	               ...
	               if(customerCart == null){
	            	   if(sessionShoppingCart.getCustomerId()==null) {//saved shopping cart does not belong to a customer
	                    ...
	                    if(sessionShoppingCart.getCustomerId()==null) {
	                    	//saved shopping cart does not belong to a customer
	                    	//assign it to logged in user
	                    	LOG.debug( "Customer shopping cart as well session cart is available, merging carts" );
	                    	/**
	                    	 * Empty cart
	                    	 */
	                    	if(!sessionShoppingCart.getLineItems().isEmpty()){
		                    	for(ShoppingCartItem cartItem : customerCart.getLineItems()){
		                    		//ShoppingCartData shoppingCartData=shoppingCartFacade.removeCartItem(cartItem.getId(), customerCart.getShoppingCartCode(), store,language);
		                    		removeCartItem(cartItem.getId(), customerCart, store, language);
		                    		/*if(CollectionUtils.isEmpty(shoppingCartData.getShoppingCartItems())) {
		                    			shoppingCartFacade.deleteShoppingCart(shoppingCartData.getId(), store);
		                    			removeCartItem(itemID, cartId, store, language)
		                    		}*/
		                    	}
	                    	}
   ...
   @Override
	...
	public void authenticate(Customer customer, String userName, String password) throws Exception {
			...
			Authentication authenticationToken =
                new UsernamePasswordAuthenticationToken(customer.getNick(), password, authorities);
	...
	public ShoppingCartData removeCartItem( final Long itemID, final ShoppingCart cartModel ,final MerchantStore store,final Language language )
	        throws Exception
	    {
	...
	@Override
	public boolean checkIfUserExistsWithSamePhone(SecuredShopPersistableCustomer customer, MerchantStore store) throws Exception {
        if ( customer != null && StringUtils.isNotBlank( customer.getBilling().getPhone() ) && store != null )
        {
            Customer customer1 = customerService.getByPhone(customer.getBilling().getPhone(), store.getId());
            if ( customer1 != null )
            {
                LOG.info( "Customer with mobile number {} already exists for store {} ", customer.getBilling().getPhone(), store.getStorename() );
                return true;
            }
            
            LOG.info( "No customer found with mobile number {} for store {} ", customer.getBilling().getPhone(), store.getStorename());
            return false;

        }
        LOG.info( "Either mobile number is empty or we have not found any value for store" );
        return false;
    }


	@Override
	public boolean checkIfUserExistsWithSameEmail(SecuredShopPersistableCustomer customer, MerchantStore store)
			throws Exception {
        if ( customer != null && StringUtils.isNotBlank( customer.getEmailAddress() ) && store != null )
        {
            Customer customer1 = customerService.getByEmail(customer.getEmailAddress(), store.getId());
            if ( customer1 != null )
            {
                LOG.info( "Customer with email {} already exists for store {} ", customer.getEmailAddress(), store.getStorename() );
                return true;
            }
            
            LOG.info( "No customer found with email {} for store {} ", customer.getEmailAddress(), store.getStorename());
            return false;

        }
        LOG.info( "Either email is empty or we have not found any value for store" );
        return false;
    }
