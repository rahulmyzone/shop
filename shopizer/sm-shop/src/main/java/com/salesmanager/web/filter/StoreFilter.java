...
public class StoreFilter extends HandlerInterceptorAdapter {
			...
			try {
				...
				if(anonymousCustomer==null) {
					...
					try {
						...
						com.salesmanager.core.business.common.model.Address geoAddress = customerService.getCustomerAddress(store, "123.237.179.64");
						if(geoAddress!=null) {
							address = new Address();
							address.setCountry(geoAddress.getCountry());
							address.setCity(geoAddress.getCity());
							address.setZone(store.getZone().getCode());
							System.out.println("Zone------------------>>>>>>"+store.getZone());
				...
				this.setRecommendedDeals(store, language, request);
	...
	@SuppressWarnings("unchecked")
	private void setRecommendedDeals(MerchantStore store, Language language, HttpServletRequest request) throws Exception{
		StringBuilder recomDealsKey = new StringBuilder();
		recomDealsKey
		.append(store.getId())
		.append("_")
		...
		Map<String, List<ReadableProduct>> objects = null;
		List<ReadableProduct> loadedDeals = null;
		if(store.isUseCache()) {
			objects = (Map<String, List<ReadableProduct>>) webApplicationCache.getFromCache(recomDealsKey.toString());
			
			if(objects==null) {
				//load categories
				loadedDeals = (List<ReadableProduct>)request.getAttribute(ProductRelationshipType.RECOMMENDED_DEALS.name());
				objects = new ConcurrentHashMap<String, List<ReadableProduct>>();
				objects.put(language.getCode(), loadedDeals);
				webApplicationCache.putInCache(recomDealsKey.toString(), objects);
				
			} else {
				loadedDeals = objects.get(language.getCode());
			}
		...
		if(recomDealsKey!=null) {
			request.setAttribute(Constants.REQUEST_RECOMMENDED_DEALS, loadedDeals);
		}
	}
