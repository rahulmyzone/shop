...
import com.salesmanager.core.business.search.service.SolrSearchService;
...
public class SearchController {
	...
	public String autocomplete(@RequestParam("q") String query, @PathVariable String store, @PathVariable final String language, Model model, HttpServletRequest request, HttpServletResponse response)  {
		...
		List<IndexProduct> keywords = solrSearchService.searchForProducts(req.getCollectionName(), query, 10);
		//List<IndexProduct> keywords = solrSearchService.searchForProducts(req.getCollectionName(), query, AUTOCOMPLETE_ENTRIES_COUNT);
		JSONArray jsonArray = new JSONArray();
		for(IndexProduct in : keywords){
			JSONObject jsonObject = in.toJSONObject();
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value={"/shop/search/search.html"}, method=RequestMethod.GET)
