...
import com.salesmanager.core.business.catalog.promotions.service.OrderCodeService;
...
public class OrderFacadeImpl implements OrderFacade {
	...
	private OrderTotalSummary calculateOrderTotal(MerchantStore store, Customer customer, PersistableOrder order, Language language) throws Exception {
		...
		if(order instanceof ShopOrder) {
			...
			if(o.getPromotionalVoucher() != null){
				summary.setPromotionalVoucher(o.getPromotionalVoucher());
				summary.setPromotionApplied(true);
			}
	...
	private Order processOrderModel(ShopOrder order, Customer customer, Transaction transaction, MerchantStore store,
		...
		try {
			...
			for(ShoppingCartItem item : shoppingCartItems) {
				...
				OrderCode code = orderCodeService.getOrderCode(false, false);
				orderProduct.setOrderCode(code);
				orderProducts.add(orderProduct);
				code.setIssued(true);
				orderCodeService.update(code);
			}
			...
			if(PaymentType.ATOM.name().equals(paymentType)){
				//check for previous transaction
				if(transaction==null) {
					throw new ServiceException("payment.error");
				}
			...
			/*OrderCode code = this.orderCodeService.getOrderCode(false, false);
			modelOrder.setOrderCode(code);*/
	...
	public ShippingQuote getShippingQuote(Customer customer, ShoppingCart cart,
     ...
     private ReadableOrderList populateOrderList(final OrderList orderList,final MerchantStore store, final Language language){
        ...
        for (Order order : orders) {
            ...
            /*readableOrder.setOrderCode(order.getOrderCode());*/
	...
	public ReadableOrder getReadableOrder(Long orderId, MerchantStore store,
		...
		Order modelOrder = orderService.getOrder(orderId);
		if(modelOrder==null) {
		...
		/*readableOrder.setOrderCode(modelOrder.getOrderCode());*/
		readableOrder.setProducts(orderProducts);
