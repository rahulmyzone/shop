...
public class ShopProductController {
	...
	public String display(final String reference, final String friendlyUrl, Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
		...
		if(product.getProductImage() != null){
			request.setAttribute(Constants.SE_IMAGE, product.getProductImage());
		}
		...
		/*if(store.isUseCache()) {
			...
			//relatedItems = relatedItems(store, product, language);
		/*}*/
		//List<List<ReadableProduct>> relatedProducts = new ArrayList<List<ReadableProduct>>();
		/*if(relatedItems != null){
				int numItems = 4;
				int numPages = Utility.getNumPages(relatedItems.size(), numItems);
				for(int i=0; i<numPages; i++){
					List<ReadableProduct> pr = new ArrayList<>();
					for(int j = 0; j<numItems; j++){
						try{
						pr.add(relatedItems.get(i*numItems+j));
						}catch(Exception e){
							e.printStackTrace();
							break;
						}
					}
					relatedProducts.add(pr);
				}
		}
		model.addAttribute("relatedProducts",relatedProducts);	*/
		Set<ProductAttribute> attributes = product.getAttributes();
		
		//model.addAttribute("cProducts",relatedItems);	
