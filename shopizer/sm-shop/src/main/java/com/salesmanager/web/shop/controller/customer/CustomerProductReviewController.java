...
public class CustomerProductReviewController extends AbstractController {
	...
	@PreAuthorize("hasRole('AUTH_CUSTOMER')")
	@RequestMapping(value={"/submit-review.html"},  method = { RequestMethod.POST }, produces="application/json", consumes="application/x-www-form-urlencoded")
	public @ResponseBody ReadableProductReview submitReview(@RequestParam("review") String value, @RequestParam("rating") String rating, @RequestParam("productId") String productId, final HttpServletRequest request, final HttpServletResponse response){
		MerchantStore store = getSessionAttribute(Constants.MERCHANT_STORE, request);
	    Language language = super.getLanguage(request);
	    ReadableProductReview rev = new ReadableProductReview();
		if(value != null && rating != null){
			try{
				Customer customer =  customerFacade.getCustomerByUserName(request.getRemoteUser(), store);
				ProductReview review = new ProductReview();
				Product product = productService.getByCode(productId, language);
				if(product == null){
					rev.setAjaxResponse(AjaxResponse.RESPONSE_STATUS_FAIURE);
					return rev;
				}
		        review.setCustomer(customer);
		        review.setProduct(product);
		        ...
		        Set<ProductReviewDescription> l = new HashSet<ProductReviewDescription>();
		        desc.setProductReview(review);
		        l.add(desc);
		        review.setDescriptions(l);
		        review.setReviewRating(Double.parseDouble(rating));
		        review.setStatus(0);
		        productReviewService.create(review);
		        ReadableProductReviewPopulator reviewPopulator = new ReadableProductReviewPopulator();
				reviewPopulator.populate(review, rev, store, language);
		        rev.setAjaxResponse(AjaxResponse.RESPONSE_STATUS_SUCCESS);
			}catch (Exception e) {
				e.printStackTrace();
			}
