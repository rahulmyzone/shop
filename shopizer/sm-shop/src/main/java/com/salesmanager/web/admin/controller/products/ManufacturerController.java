...
public class ManufacturerController {
	...
	private String displayManufacturer(Long manufacturerId, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		MerchantConfiguration manufacturerFBConfiguration = null;
		MerchantConfiguration manufacturerTwitterConfiguration = null;
		MerchantConfiguration manufacturerGPlusConfiguration = null;
		MerchantConfiguration manufacturerPinterestConfiguration = null;
		
		manufacturerFBConfiguration = merchantConfigurationService.getManufacturerConfiguration(Constants.KEY_FACEBOOK_PAGE_URL,manufacturer.getManufacturer());
		manufacturerTwitterConfiguration = merchantConfigurationService.getManufacturerConfiguration(Constants.KEY_TWITTER_HANDLE,manufacturer.getManufacturer());
		manufacturerGPlusConfiguration = merchantConfigurationService.getManufacturerConfiguration(Constants.KEY_GOOGLE_PLUS_URL,manufacturer.getManufacturer());
		manufacturerPinterestConfiguration = merchantConfigurationService.getManufacturerConfiguration(Constants.KEY_PINTEREST_PAGE_URL,manufacturer.getManufacturer());
		
		if(manufacturerFBConfiguration == null){
			manufacturerFBConfiguration = new MerchantConfiguration();
			manufacturerFBConfiguration.setKey(Constants.KEY_FACEBOOK_PAGE_URL);
			manufacturerFBConfiguration.setMerchantConfigurationType(MerchantConfigurationType.CONFIG);
		}
		configs.add(manufacturerFBConfiguration);
		
		if(manufacturerTwitterConfiguration == null){
			manufacturerTwitterConfiguration = new MerchantConfiguration();
			manufacturerTwitterConfiguration.setKey(Constants.KEY_TWITTER_HANDLE);
			manufacturerTwitterConfiguration.setMerchantConfigurationType(MerchantConfigurationType.CONFIG);
		}
		configs.add(manufacturerTwitterConfiguration);
		
		if(manufacturerGPlusConfiguration == null){
			manufacturerGPlusConfiguration = new MerchantConfiguration();
			manufacturerGPlusConfiguration.setKey(Constants.KEY_GOOGLE_PLUS_URL);
			manufacturerGPlusConfiguration.setMerchantConfigurationType(MerchantConfigurationType.CONFIG);
		}
		configs.add(manufacturerGPlusConfiguration);
		
		if(manufacturerPinterestConfiguration == null){
			manufacturerPinterestConfiguration = new MerchantConfiguration();
			manufacturerPinterestConfiguration.setKey(Constants.KEY_PINTEREST_PAGE_URL);
			manufacturerPinterestConfiguration.setMerchantConfigurationType(MerchantConfigurationType.CONFIG);
		}
		configs.add(manufacturerPinterestConfiguration);
		
		model.addAttribute("languages",languages);
		model.addAttribute("manufacturer", manufacturer);
		
		ConfigListWrapper configWrapper = new ConfigListWrapper();
		configWrapper.setManufacturerConfigs(configs);
		//model.addAttribute("configuration",configWrapper);
		manufacturer.setWrapper(configWrapper);
...
//		}
		...
		newManufacturer.setLatitude(manufacturer.getManufacturer().getLatitude());
		newManufacturer.setLongitude(manufacturer.getManufacturer().getLongitude());
...
//			manufacturer.setProductImage(manufacturerImage);
		 ...
		 */
		List<MerchantConfiguration> configs = manufacturer.getWrapper().getManufacturerConfigs();
		if(configs != null){
			for(MerchantConfiguration configuration : configs){
				configuration.setManufacturer(manufacturer.getManufacturer());
				if(!StringUtils.isBlank(configuration.getValue())) {
					...
					MerchantConfiguration config = merchantConfigurationService.getManufacturerConfiguration(configuration.getKey(), manufacturer.getManufacturer());
					if(config!=null) {
