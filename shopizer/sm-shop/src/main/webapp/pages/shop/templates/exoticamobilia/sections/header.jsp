...
<script src="<c:url value="/resources/templates/exoticamobilia/js/js/bloodhound.min.js" />"></script>
<script src="<c:url value="/resources/templates/exoticamobilia/js/js/typeahead.bundle.min.js" />"></script>
...
$(document).ready(function() { 
   ...
   var searchElements = new Bloodhound({
	    ...
	    remote: {url:'<c:url value="/services/public/search/${requestScope.MERCHANT_STORE.code}/${requestScope.LANGUAGE.code}/autocomplete.html"/>?q=%QUERY',
	    	wildcard: '%QUERY'
   			},
		limit:10
	});
   searchElements.initialize();
	...
	var searchTemplate =  Hogan.compile([
	                 '<div><img class="suggest-img" src="/static/{{store}}/PRODUCT/{{sku}}/{{image}}"/><p class="menu-text"><a href="http://localhost:8080/shop/product/{{url}}.html">{{name}}</a></p>' + '<p class="price menu-text"> {{price}}</p></div>'
	             ].join(''));
	
	var footerTemplate = function (data) {
	    return '<div class="show-all"><a href="#">see all results for </p></div>'
	  }
    //full view search
...
<script type="text/html" id="miniCartTemplate">
							<li>
								...
								<table class="table table-hover">
									...
									<tbody>
										...
										<tr id="{{productId}}" class="cart-product">
											...
											<td class="cart-product">{{quantity}} x {{name}}</td>
								...
								<div class="panel-body text-right">	
									    <button type="button" onclick="viewShoppingCartPage();" class="btn btn-default get"><s:message code="label.checkout" text="Checkout"/></button>
								</div>
								{{/code}}
								{{^code}}
...
<script type="text/html" id="miniCartSummaryTemplate">
		...
		{{^code}}
		<span class="no-responsive uppercase"><s:message code="label.cart" text="Cart"/></span> (0)
		{{/code}}
		{{#code}}
		<span class="no-responsive uppercase"><s:message code="label.cart" text="Cart"/></span> ({{quantity}})
		{{/code}}
</script>
        ...
        <ul id="signinPane" class="dropdown-menu dropdown-menu-right dropdown-animation">
			<li>
				...
				<form class="login-form" id="login" method="post" accept-charset="UTF-8">
					...
					<span></span>
					<br/>
					<a id="registerLink" onClick="javascript:location.href='<c:url value="/shop/customer/registration.html" />';"  role="button" class="" data-toggle="modal"><s:message code="label.register.notyetregistered" text="Not yet registered ?" /></a>
					<br/>
				</form>
			</li>
		</ul>
					...
					<div class="col-sm-6 ">
						<div class="contactinfo">
							<ul class="nav nav-pills">
								...
								<li><a ><i class="fa fa-envelope"></i> info@domain.com</a></li>
							</ul>
						</div>
					</div>
					<div class="col-sm-6">
						...
						<div class="shop-menu pull-right">
							<ul class="nav navbar-nav">
								<sec:authorize access="hasRole('AUTH_CUSTOMER') and fullyAuthenticated">
									<li><a ><i class="fa fa-star"></i> Wishlist</a></li>
								</sec:authorize>
								<li><a href='<c:url value="${requestScope.contextPath}/shop/cart/shoppingCart.html" />'><i class="fa fa-crosshairs"></i> Checkout</a></li>
								<li>
									<button id="open-cart" type="button" class="btn dropdown-toggle" data-toggle="dropdown">
										<i class="fa fa-shopping-cart no-desktop"></i> 
										<span id="miniCartSummary"></span></button>
									<ul id="miniCartDetails" class="dropdown-menu dropdown-menu-right dropdown-animation"></ul>
								</li>
								<sec:authorize access="!hasRole('AUTH_CUSTOMER') and !fullyAuthenticated">
									<li><a href='<c:url value="${requestScope.contextPath}/shop/customer/customLogon.html" />'><i class="fa fa-lock" ></i> Login</a></li>
								</sec:authorize>
								<sec:authorize access="hasRole('AUTH_CUSTOMER') and fullyAuthenticated">
									<li><a href="/shop/customer/dashboard.html"><i class="fa fa-lock" ></i> Welcome <sec:authentication property="principal.username"/></a></li>
								</sec:authorize>
								<sec:authorize access="hasRole('AUTH_CUSTOMER') and fullyAuthenticated">
									<li><a href='<c:url value="/shop/j_spring_security_logout" />'><i class="fa fa-lock" ></i> Logout</a></li>
								</sec:authorize>
							</ul>
						</div>
					</div>
				...
				<div class="row">
					...
					<div class="col-sm-6 col-xs-12">
						<div class="search_box pull-right">
							<form action="/shop/search/search.html" method="POST">
							
							<input class="typeahead search-input" id="searchField" name="q" type="text" placeholder="Search the deals..." autocomplete="off" spellcheck="false" dir="auto" value='<c:out value="${q}"/>'/>
