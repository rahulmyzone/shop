	...
	<div class="tab-content">
		<div class="tab-pane active" id="catalogue-section">
           <div class="sm-ui-component">
				...
				<h3>
					<s:message code="label.promotions.category.association" text="Associate to categories" />
				</h3>
				
				
			
			<c:url var="addCategory" value="/admin/promotions/categories/addCategory.html" />
			<form:form method="POST" enctype="multipart/form-data" commandName="voucher" action="${addCategory}">
				...
				<br/>
				<strong><c:out value="${voucher.sku}"/></strong>
				<br/><br/>
					<div class="control-group">
						<label><s:message code="label.promotionsEdit.categoryname"
								text="Category" /></label>
						<div class="controls">
							<form:select path="id">
								<form:options items="${categories}" itemValue="id"
									itemLabel="descriptions[0].name" />
							</form:select>
							<span class="help-inline"><form:errors path="id"
									cssClass="error" /></span>
						</div>
					</div>
			...
			<input type="hidden" name="productId" value="${voucher.id}">
			<div class="form-actions">
                  		<div class="pull-right">
                  			<button type="submit" class="btn btn-success"><s:message code="label.generic.add" text="Add"/></button>
                  		</div>
				...
				<c:set value="/admin/promotions/categories/paging.html?id=${voucher.id}" var="pagingUrl" scope="request" />
				<c:set value="/admin/promotions/categories/remove.html?voucherId=${voucher.id}" var="removeUrl" scope="request" />
				<c:set value="/admin/promotions/categories/list.html?id=${voucher.id}" var="refreshUrl" scope="request" />
