...
function getItemLabel(quantity) {
	var labelItem = '<s:message code="label.generic.cart" text="item" />';
	if (quantity > 1) {
...
function getRegistrationErrorLabel(response){
	var msg='';
	for(var obj in response.response.data[0]){
		msg = msg+response.response.data[0][obj]+'<br/>';
	}
	return msg;
}
...
function cartInfoLabel(cart){
	 ...
	 <c:when test="${requestScope.CONFIGS['displayFullMiniCartInfo'] == true}">
		 $("#cartinfo").html('<span id="cartqty">(' + cart.quantity + ')</span>&nbsp;<span id="cartprice">' + cart.total + '</span>');
	 </c:when>
	 <c:otherwise>
	 	$("#cartinfo").html('<span id="cartqty">(' + cart.quantity + ')</span>');
	 </c:otherwise>
	 </c:choose>
}

function cartSubTotal(cart) {
	return '<span class="text"><s:message code="label.subtotal" text="Sub-total" /> :</span><span  class="price"><i class="fa fa-inr"></i> ' + cart.subTotal + '</span>';
}
