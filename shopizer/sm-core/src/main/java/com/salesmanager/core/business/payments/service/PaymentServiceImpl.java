	...
	public List<PaymentMethod> getAcceptedPaymentMethods(MerchantStore store) throws ServiceException {
		...
		for(String module : modules.keySet()) {
			...
			if(config.isActive()) {
				...
				}else 	if(md.getType().toUpperCase().equalsIgnoreCase(PaymentType.CITRUS.name())) {
					type = PaymentType.CITRUS;
				}else 	if(md.getType().toUpperCase().equalsIgnoreCase(PaymentType.ATOM.name())) {
					type = PaymentType.ATOM;
				}else 	if(md.getType().toUpperCase().equalsIgnoreCase(PaymentType.PAYTM.name())) {
