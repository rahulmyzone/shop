...
public class CustomerDAOImpl extends SalesManagerEntityDaoImpl<Long, Customer> implements CustomerDAO {
	...
	public Customer getByNick(String nick){
		...
		query.from(qCustomer)
			...
			.leftJoin(qCustomer.delivery.country).fetch()
			.leftJoin(qCustomer.delivery.zone).fetch()
			.leftJoin(qCustomer.defaultLanguage).fetch()
		...
		Customer c = query.uniqueResult(qCustomer);
		return c;
	}
	...
	public Customer getByNick(String nick, int storeId){
		...
		query.from(qCustomer)
			...
			.leftJoin(qCustomerOption.descriptions).fetch()
			.leftJoin(qCustomerOptionValue.descriptions).fetch()
			.where((qCustomer.nick.eq(nick).or(qCustomer.mobile.eq(nick).or(qCustomer.emailAddress.eq(nick)))).and(qMerchantStore.id.eq(storeId)));
		Customer c = query.uniqueResult(qCustomer);
		return c;
	...
	@Override
	public Customer getByEmail(String email, int storeId) {
		QCustomer qCustomer = QCustomer.customer;
		QMerchantStore qMerchantStore = QMerchantStore.merchantStore;
		QCountry qCountry = QCountry.country;
		QZone qZone = QZone.zone;
		...
		query.from(qCustomer)
			...
			.leftJoin(qCustomerOption.descriptions).fetch()
			.leftJoin(qCustomerOptionValue.descriptions).fetch()
			.where(qCustomer.emailAddress.eq(email).and(qMerchantStore.id.eq(storeId)));
		Customer c = query.uniqueResult(qCustomer);
		return c;
	...
	public Customer getByPhone(String phone, int storeId) {
		...
		query.from(qCustomer)
			...
			.leftJoin(qCustomerOption.descriptions).fetch()
			.leftJoin(qCustomerOptionValue.descriptions).fetch()
			.where(qCustomer.mobile.eq(phone).and(qMerchantStore.id.eq(storeId)));
		Customer c = query.uniqueResult(qCustomer);
		return c;
