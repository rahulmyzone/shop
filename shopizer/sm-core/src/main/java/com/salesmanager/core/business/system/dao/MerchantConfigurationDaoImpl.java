...
public class MerchantConfigurationDaoImpl extends SalesManagerEntityDaoImpl<Long, MerchantConfiguration>
	...
	@Override
	public MerchantConfiguration getManufacturerConfiguration(String key, Manufacturer manufacturer) {
		...
		if(manufacturer.getId() !=  null){
			...
			JPQLQuery query = new JPAQuery (getEntityManager());
			query.from(qMerchantConfiguration)
				.innerJoin(qMerchantConfiguration.manufacturer).fetch()
				.where(qMerchantConfiguration.manufacturer.id.eq(manufacturer.getId())
				.and(qMerchantConfiguration.key.eq(key)));
