...
import com.salesmanager.core.Util.CoreUtils;
import com.salesmanager.core.business.catalog.product.model.price.FinalPrice;
import com.salesmanager.core.business.catalog.promotions.model.PromotionalVoucher;
import com.salesmanager.core.business.catalog.promotions.model.PromotionalVoucher.PromotionApplied;
...
public class OrderServiceImpl  extends SalesManagerEntityServiceImpl<Long, Order> implements OrderService {
    ...
    private OrderTotalSummary caculateOrder(final OrderSummary summary, final Customer customer, final MerchantStore store, final Language language) throws Exception {
        ...
        for(ShoppingCartItem item : summary.getProducts()) {
        	...
        	if(item.getPromotionalVoucher() != null){
        		if(item.getPromotionalVoucher().getPromotionApplied() == PromotionApplied.PERCENT){
        			item.setDiscountedPrice(CoreUtils.subtractPercent(item.getItemPrice(), item.getPromotionalVoucher().getValue()));
        			st = item.getDiscountedPrice().multiply(new BigDecimal(item.getQuantity()));
        		}else if(item.getPromotionalVoucher().getPromotionApplied() == PromotionApplied.AMOUNT){
        			item.setDiscountedPrice(CoreUtils.subtract(item.getItemPrice(), item.getPromotionalVoucher().getValue()));
        			st = item.getDiscountedPrice().multiply(new BigDecimal(item.getQuantity()));
        		}
        	}else{
        ...
        totalSummary.setOrderSummary(summary);
	...
	@Override
	public List<OrderProduct> listByPromoCode(String code) {
		return orderDao.listByPromoCode(code);
	}
