...
public class ODSInvoiceModule implements InvoiceModule {
	...
	private final static int BILLTO_ROW_START = 3;
	private final static int BILLTO_ROW_END = 6;
	
	private final static int USER_EMAIL_ROW = 7;
	
	private final static int USER_PHONE_ROW = 8;
	
	private final static int PAYMENT_TYPE_ROW = 14;
	
	private final static int PRODUCT_ROW_START = 18;
	...
	public ByteArrayOutputStream createInvoice(MerchantStore store, Order order, Language language) throws Exception {
			...
			*/
			...
			sheet.setValueAt(format.format(order.getDatePurchased()), 4, 2);
			
			//invoice number
			sheet.setValueAt(order.getId(), 4, 3);
			...
			if(!StringUtils.isBlank(order.getBilling().getCity())) {
				sheet.setValueAt(order.getBilling().getCity(), 0, billToCell);
				billToCell ++;
			}
			
			if(order.getBilling().getCountry()!=null) {
				sheet.setValueAt(order.getBilling().getCountry().getName(), 0, billToCell);
				billToCell ++;
			}
			...
			if(!StringUtils.isBlank(order.getCustomerEmailAddress())) {
				sheet.setValueAt(order.getCustomerEmailAddress(), 0, USER_EMAIL_ROW);
			}
			
			if(!StringUtils.isBlank(order.getBilling().getTelephone())) {
				sheet.setValueAt(order.getBilling().getTelephone(), 0, USER_PHONE_ROW);
			}
			
			if(order.getPaymentType() != null && !StringUtils.isBlank(order.getPaymentType().name())){
				sheet.setValueAt(order.getPaymentType().name(), 0, PAYMENT_TYPE_ROW);
			}
			...
			for(OrderProduct orderProduct : orderProducts) {
				...
				/*sheet.setValueAt(orderProduct.getSku(), 0, productCell);*/
				String orderProductName = ProductUtils.buildOrderProductDisplayName(orderProduct);
				
				String productName = WordUtils.wrap(orderProductName.toString(), 40, "\n", true);
				if(productName.indexOf("\n") != -1){
					String[] strs = productName.split("\n");
					for(String s: strs){
						sheet.setValueAt(s, 0, productCell);
						productCell++;
					}
				}else{
					
				}
				sheet.setValueAt(productName, 0, productCell);
				if(orderProduct.getOrderCode() != null){
					sheet.setValueAt(orderProduct.getOrderCode().getCode(), 1, productCell);
				}
				
				int quantity = orderProduct.getProductQuantity();
				sheet.setValueAt(quantity, 2, productCell);
				
				String amount = priceUtil.getStoreFormatedAmountWithCurrency(store, orderProduct.getOneTimeCharge());
				sheet.setValueAt(amount, 3, productCell);
				
				
				
				String t = priceUtil.getStoreFormatedAmountWithCurrency(store, priceUtil.getOrderProductTotalPrice(store, orderProduct));
				sheet.setValueAt(t, 4, productCell);
			...
			for(OrderTotal orderTotal : totals) {
				...
				sheet.setValueAt(totalName, 3, productCell);
				sheet.setValueAt(totalValue, 4, productCell);
				productCell++;
			}
