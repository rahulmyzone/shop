...
public class OrderDaoImpl  extends SalesManagerEntityDaoImpl<Long, Order> implements OrderDao {
	...
	public List<OrderProduct> listByPromoCode(String code) {
		...
		qs.append("select distinct op from OrderProduct as op ");
		qs.append("join fetch op.order o ");
		qs.append("join fetch op.promotionalVoucher pv ");
		qs.append("left join fetch op.orderAttributes oa ");
		qs.append("left join fetch op.prices pr ");
		qs.append("where pv.promoCode in (:code) ");
	...
	@Override
	public Order getByCodeOrder(OrderCode code) {
		Order order = null;
		StringBuilder qs = new StringBuilder();
		qs.append("select distinct o from Order as o ");
